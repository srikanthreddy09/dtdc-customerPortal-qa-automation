...
Launch Browser
    ...
    Run Keyword If    '${browser_name}'=='Firefox' or '${browser_name}'=='firefox' or '${browser_name}'=='ff'    Open Firefox Browser    ${url}    ${HEADLESS}
    Run Keyword If    '${browser_name}'=='ie' or '${browser_name}'=='Internet Explorer' or '${browser_name}'=='internet explorer'    Open Ie Browser    ${url}    ${HEADLESS}
    Run Keyword If    '${browser_name}'=='edge' or '${browser_name}'=='Edge'    Open Edge Browser    ${url}    ${HEADLESS}

Launch Browser and Navigate to URL
    [Arguments]    ${url}    ${browser_name}    ${clear_cache}=False
    ${session}    Run Keyword And Return Status    Get Session Id
    ${url}    Run Keyword If    "${ENVIRONMENT}"=="Staging" or "${ENVIRONMENT}"=="STAGING" or "${ENVIRONMENT}"=="staging"    Set Variable    ${SHIPSY_URL}
